...
<body class="bg-gray-100">
    ...
    <div class="container mx-auto py-10">
        <div>
            <a href="{{ route('profile.index') }}"> <button type="button" class="text-white bg-gradient-to-r from-cyan-400 via-cyan-500 to-cyan-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-cyan-300 dark:focus:ring-cyan-800 shadow-lg shadow-cyan-500/50 dark:shadow-lg dark:shadow-cyan-800/80 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2">Back to the Profile</button>
            </a>
         </div>
        <div class="bg-gray-200 rounded-lg shadow-lg flex flex-col md:flex-row justify-center items-center md:space-x-8 p-6">
            <div class="md:w-1/3">
                <img src="{{ $users->getFirstMediaUrl('image') }}" class="rounded-full w-60 h-60 md:w-72 md:h-72 shadow-xl" alt="">
                </div>
               <form action="{{ route('profile.update', $users->id) }}" method="POST" class="md:w-2/3 pt-4 md:pt-0" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div>
                    <input type="text" name="name" value="{{ $users->name }}" class="input" placeholder="Address">

                </div>

                <div class="flex flex-col md:flex-row justify-center md:justify-between mt-4 md:mt-2 space-y-4 md:space-y-0">
                    <input type="text" name="address" value="{{ $users->address }}" class="input" placeholder="Address">
                    <input type="text" name="phone" value="{{ $users->phone }}" class="input" placeholder="Phone">
                </div>
